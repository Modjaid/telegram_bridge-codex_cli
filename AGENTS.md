...
- Bridge slash commands keep priority over the active `/schedule` dialog, so commands such as `/projects` are handled by the Bridge instead of being submitted to Codex.
